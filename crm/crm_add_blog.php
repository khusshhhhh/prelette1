...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $seo_url = strtolower(str_replace(' ', '-', trim($title))); // Auto-generate SEO URL
    ...
    if ($check_stmt->num_rows > 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO blogs_html (title, author, date, content, seo_url, image_url1) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssss", $title, $author, $date, $content, $seo_url, $image_url1);
...
<div class="container mt-4">
    ...
    <form method="POST">
        ...
        <div class="mb-3">
            <input type="text" name="image_url1" class="form-control" placeholder="Main Image URL" required>
        </div>
        <div class="mb-3">
            <textarea name="content" class="form-control" rows="20" placeholder="Enter HTML script here"
                required></textarea>
        </div>
